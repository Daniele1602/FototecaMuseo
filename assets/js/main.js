	...
	var _ = {
		...
		initViewer: function () {
			...
			_.$thumbnails.children()
				.each(function () {
					...
					//var url = encodeURI("details.html?"+s.url.split("//")[2]);
					var code = $this.attr("code")
					s.code = code
					s.$slide = $('<div class="slide">'+
					'<div id="icon" onclick="ShowInfo($(\'.image\').attr(\'code\'))">'+
					'<img src="assets/Images/info-icon.png">'+
					'</div>'+'<div class="image" code = "'+code+'"></div></div>');
		...
		toggle: function () {
			...
			if (_.$body.hasClass('fullscreen')) {
				_.show();
				$("#info").css("left", "25%");
				$("#errorMessage").css("left", "25%");
			}
			else {
				_.hide();
				$("#info").css("left", "28%");
				$("#errorMessage").css("left", "35%");
			}
		},
